...
# Then we draw: {R,R,R}
# We are choosing "0" black out of 3.
trueProb[1] = choose(ballsToDraw, 0) * (totalRed / totalBalls) * ((totalRed -
                                                                       1) / (totalBalls - 1)) * ((totalRed - 2) / (totalBalls - 2))
...
# 1st Black on 1st Pick: {B,B,R}, {B,R,B}
# 1st Black on 2nd Pick: {R,B,B}
# Each of these outcomes has the same probability (since order doesn't matter)
# P(Draw 2 Black) = choose(3,2)*P(R,B,B)
...
# 1st Black on 1st Pick: {B,B,B}
# Each of these outcomes has the same probability (since order doesn't matter)
# P(Draw 3 Black) = choose(3,3)*P(B,B,B)
...
plot(
    c(0:ballsToDraw),
    numBlackDrawnTally / numTrials,
    col = 'blue',
    xlab = "Number of Black Balls Drawn",
    ylab = "Probability",
    type = 'b',
    main = "Probability of Drawing Balls from an Urn"
)
